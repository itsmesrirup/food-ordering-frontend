...
const CheckoutUI = ({ 
    ...
    cartItems, totalPrice, updateQuantity, isCurrentlyClosed, getMinAllowedDate // ✅ ADDED updateQuantity prop
}) => {
    return (
        <Box>
            ...
            >
                ...
                {/* ✅ Hide ASAP if Lead Time > 0 (e.g., Bakeries) */}
                {(!currentRestaurant || currentRestaurant.advanceOrderLeadTimeHours === 0) && (
                    ...
                    <DatePicker
                        ...
                        customInput={
                            <TextField 
                                ...
                                sx={{ '& input': { textOverflow: 'ellipsis', cursor: 'pointer' } }}
                            />
                        }
                    />
                </Box>
...
function CheckoutPage() {
    ...
    const getMinAllowedDate = () => {
        ...
        const leadTimeMs = (currentRestaurant?.advanceOrderLeadTimeHours || 0) * 60 * 60 * 1000;
        // Base 20-min prep buffer + Bakery Lead Time
        return new Date(currentDate.getTime() + (20 * 60000) + leadTimeMs);
    };

    const getNextValidPickupTime = () => {
        let checkTime = getMinAllowedDate();
        
        // Round up to the next 15-minute mark (e.g., 10:04 becomes 10:15)
        ...
        // Scan forward in 15-minute increments (up to 7 days) until we hit an open slot
        for (let i = 0; i < 24 * 4 * 7; i++) {
            if (isRestaurantOpen(checkTime, currentRestaurant?.openingHoursJson)) {
                return checkTime; // Found it!
            }
            checkTime = new Date(checkTime.getTime() + 15 * 60000); // Add 15 mins
        }
    ...
    const filterPassedTime = (time) => {
        ...
        const minTime = getMinAllowedDate();
    ...
    useEffect(() => {
        if (currentRestaurant && currentRestaurant.openingHoursJson) {
            ...
            if (!currentlyOpen) {
                ...
                // ✅ USE THE NEW SCANNER
                const nextValidSlot = getNextValidPickupTime();
                if (nextValidSlot) {
                    setSelectedDate(nextValidSlot);
                }
            }
        }
    }, [currentRestaurant]);
        ...
        <Container maxWidth="sm" sx={{ mt: 4, mb: 10 }}>
            ...
            <Paper elevation={4} sx={{ p: { xs: 3, md: 4 }, borderRadius: 3 }}>
                ...
                <CheckoutUI 
                    ...
                    getMinAllowedDate={getMinAllowedDate}
                />
